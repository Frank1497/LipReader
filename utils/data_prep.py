...
class Data_Prep:
    ...
    def vid_frames(self, vid_path:str)  -> List[float]:
        x1,y1,x2,y2 = self.get_points(vid_path=vid_path)
    ...
    def tf_decoder(self, enc_data:int) -> str:
            decoder = tf.keras.backend.ctc_decode(enc_data, input_length=[75], greedy=True)[0][0].numpy()
            return tf.strings.reduce_join([self.num_to_char(decoder)])
